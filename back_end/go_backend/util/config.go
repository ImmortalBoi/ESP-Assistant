...
func PostConfig(c *gin.Context) {
	...
	code := createCode(libraries, globalDeclarations, publishMessage, messageHandler, setup)
...
func formatLibraries(libraries string) []string {
	...
	for _, library := range librarySplit {
		...
		if strings.HasSuffix(match[1], ".h") {
			// Remove ".h" if it exists
			match[1] = strings.TrimSuffix(match[1], ".h")
		}
...
func createCode(libraries []string, globalDeclarations string, publishMessage string, messageHandler string, setup string) string {
	libraryCode := ``
